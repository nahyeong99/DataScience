...
###################################################################
# read CSV
df = pd.read_csv('../../OneDrive/문서/카카오톡 받은 파일/WA_Fn-UseC_-Telco-Customer-Churn.csv')
...
# robust correlation
df_robust.corr()['Churn'].sort_values(ascending=False).plot(kind='bar',figsize=(20,5))
plt.show()

###################################
##### Split train and test data ###
###################################
X_robust1 = df_robust.drop('Churn', axis=1)
X_robust = X_robust1.values
y_robust=df_robust['Churn']

##################################################################################
robust_X_train, robust_X_test, robust_y_train, robust_y_test = \
    train_test_split(X_robust,y_robust, test_size = 0.2, shuffle=True)
...
### Robust
knn.fit(robust_X_train, robust_y_train)

# make predictions
y_pred_knn = knn.predict(robust_X_test)
y_pred_knn_prob = knn.predict_proba(robust_X_test)

# Plot model evaluations
confusion_matrix_plot(robust_X_train,robust_y_train,robust_X_test,robust_y_test,knn,y_pred_knn, 'KNN')
roc_curve_auc_score(robust_X_test, robust_y_test, y_pred_knn_prob, 'KNN')
precision_recall_curve_and_scores(robust_X_test, robust_y_test, y_pred_knn, y_pred_knn_prob, 'KNN')
...
logreg.fit(robust_X_train, robust_y_train)

# make predictions
y_pred_logreg = logreg.predict(robust_X_test)
y_pred_logreg_prob = logreg.predict_proba(robust_X_test)

# Plot model evaluations
feature_weights(X_robust1, logreg, 'Log. Regression')
confusion_matrix_plot(robust_X_train, robust_y_train, robust_X_test, robust_y_test, logreg, y_pred_logreg, 'Log. Regression')
roc_curve_auc_score(robust_X_test, robust_y_test, y_pred_logreg_prob, 'Log. Regression')
precision_recall_curve_and_scores(robust_X_test, robust_y_test, y_pred_logreg, y_pred_logreg_prob, 'Log. Regression')
...
rf.fit(robust_X_train, robust_y_train)

# make predictions

y_pred_rf = rf.predict(robust_X_test)
y_pred_rf_prob = rf.predict_proba(robust_X_test)

# Plot model evaluations

confusion_matrix_plot(robust_X_train, robust_y_train, robust_X_test, robust_y_test, rf, y_pred_rf, "Random Forest")
roc_curve_auc_score(robust_X_test, robust_y_test, y_pred_rf_prob, "Random Forest")
precision_recall_curve_and_scores(robust_X_test, robust_y_test, y_pred_rf, y_pred_rf_prob, "Random Forest")
...
knn_cv.fit(robust_X_train, robust_y_train)

# Make predictions (classes and probabilities) with the trained model on the test set
y_pred_knn_tuned = knn_cv.predict(robust_X_test)
y_pred_knn_tuned_prob = knn_cv.predict_proba(robust_X_test)
print('KNN best number of neighbors: ', knn_cv.best_params_,'\n')

confusion_matrix_plot(robust_X_train, robust_y_train, robust_X_test, robust_y_test, knn_cv, y_pred_knn_tuned, 'KNN (tuned)')
roc_curve_auc_score(robust_X_test, robust_y_test,y_pred_knn_tuned_prob, 'KNN (tuned)')
precision_recall_curve_and_scores(robust_X_test, robust_y_test, y_pred_knn_tuned, y_pred_knn_tuned_prob, 'KNN (tuned)')
...
logreg_tuned_gs.fit(robust_X_train, robust_y_train)

# Make predictions (calsses and probabilities) with the trained models on the test set.
y_pred_logreg_tuned = logreg_tuned_gs.predict(robust_X_test)
y_pred_logreg_tuned_prob = logreg_tuned_gs.predict_proba(robust_X_test)

print('Logistic Regression - Best Parameters: ', logreg_tuned_gs.best_params_)

#Plot model evaluations
confusion_matrix_plot(robust_X_train, robust_y_train, robust_X_test, robust_y_test, logreg_tuned_gs, y_pred_logreg_tuned, 'Log. Regression (tuned)')
roc_curve_auc_score(robust_X_test, robust_y_test,y_pred_logreg_tuned_prob, 'Log. Regression (tuned)')
precision_recall_curve_and_scores(robust_X_test, robust_y_test, y_pred_logreg_tuned, y_pred_logreg_tuned_prob, 'Log. Regression (tuned)')
...
rf_random_grid.fit(robust_X_train, robust_y_train)

# Make predictions (classes and probabilities) with the trained model on the test set.
y_pred_rf_tuned = rf_random_grid.predict(robust_X_test)
y_pred_rf_tuned_prob = rf_random_grid.predict_proba(robust_X_test)

print('Random Forest - Best Parameters: ', rf_random_grid.best_params_)

# Plot model evaluations
confusion_matrix_plot(robust_X_train, robust_y_train, robust_X_test, robust_y_test, rf_random_grid, y_pred_rf_tuned, 'Random Forest (tuned)')
roc_curve_auc_score(robust_X_test, robust_y_test,y_pred_rf_tuned_prob, 'Random Forest (tuned)')
precision_recall_curve_and_scores(robust_X_test, robust_y_test, y_pred_rf_tuned, y_pred_rf_tuned_prob, 'Random Forest (tuned)')
